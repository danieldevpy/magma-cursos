...
async function getModels(){
    return fetch('/model/get_models').then(async response=>{
...
async function getPreview(data_json){
    return fetch('/certificate/make_preview', {
        method: "POST",
        headers: {
            "Content-Type": "application/json" // Indica que os dados são JSON
        },
...
async function saveCertificate(data_json){
    return fetch('/certificate/make_certificate', {
        method: "POST",
        headers: {
            "Content-Type": "application/json" // Indica que os dados são JSON
        },
...
async function Certificate(state){
    const data = {}
    var next = true;
    const elements = document.querySelectorAll(`[aria-label="inputs"]`);
    elements.forEach(element=>{
        const field = element.getAttribute('data-name');
        
        var value = element.value;
        var isRequired = element.hasAttribute('required');
        if(value == "Selecione um modelo"){
            value = "";
        }
        if (isRequired && !value){
    ...
    if(state == "preview"){
        ...
        const div_previus = document.getElementById("previus");
        div_previus.style.display = "none";
        const response = await saveCertificate(data);
        const div_after = document.getElementById("after");
        div_after.style.display = "block";
        const a_link = document.getElementById("link");
        a_link.href = `/dash/history/${key}`;
    }


}

async function setOptionsCheckBox(){
    const select = document.getElementById("models");
    const datas = await getModels();
    
    datas.models.forEach(model=>{
